...
def change_mac_address(interface, mac_address):
    ...
    if verify(interface, mac_address):
        ...
        a = subprocess.run(["ifconfig", interface, "down"], stderr=subprocess.DEVNULL) # Change interface status to down
        b = subprocess.run(["ifconfig", interface, "hw", "ether", mac_address], stderr=subprocess.DEVNULL) # Change Mac Adress
        c = subprocess.run(["ifconfig", interface, "up"], stderr=subprocess.DEVNULL) # Change interface status to up

        if a.returncode:
            print(colored(f"\n[!] Error to get down {interface} interface.\n", "red"))
        elif b.returncode:
            print(colored(f"\n[!] Invalid Mac Address.\n", "red"))
        elif c.returncode:
            print(colored(f"\n[!] Error to get up {interface} interface.\n", "red"))
        else:
